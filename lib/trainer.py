...
class Trainer(object):
    def __init__(self, params=None, exp_dir=None, exp_subdir=None, use_cuda=False, multi_gpu=False):
        ...
        if self.params.gan == 'stylegan2_ffhq256':
    ...
    def get_starting_iteration(self, latent_support_sets, corpus_support_sets):
        ...
        if osp.isfile(self.checkpoint):
            ...
            for layer_idx in range(len(latent_support_sets)):
                latent_support_sets[layer_idx].load_state_dict(checkpoint_dict['latent_support_sets'][layer_idx])
    ...
    def train(self, generator, latent_support_sets, corpus_support_sets, clip_model, id_loss=None):
        ...
        Args:
            ...
            latent_support_sets : list of trainable LSS model -- interpretable latent paths model
        ...
        """
        # Save initial `latent_support_sets` model(s) as `latent_support_sets_init.pt`
        latent_support_sets_init_dict = dict()
        for layer_idx in range(len(latent_support_sets)):
            latent_support_sets_init_dict.update(
                {'layer_{}'.format(layer_idx): latent_support_sets[layer_idx].state_dict()})
        torch.save(latent_support_sets_init_dict, osp.join(self.models_dir, 'latent_support_sets_init.pt'))
        ...
        if self.use_cuda:
            ...
            for lss in latent_support_sets:
                lss.cuda().train()
        ...
        learnable_params = []
        for lss in latent_support_sets:
            learnable_params += lss.parameters()
        optimizer = torch.optim.Adam(params=learnable_params, lr=self.params.lr)
        # REVIEW: Use Ranger optimizer?
        ...
        for iteration in range(starting_iter, self.params.max_iter + 1):
            ...
            tmp_ids = torch.ones(latent_support_sets[0].num_support_sets).multinomial(
                num_samples=self.params.batch_size, replacement=False)
            target_support_sets_indices = torch.arange(latent_support_sets[0].num_support_sets)[tmp_ids]
            if self.use_cuda:
            ...
            if ('stylegan' in self.params.gan) and (self.params.stylegan_space in ('W+', 'S')):
                if self.params.stylegan_space == 'W+':
                    # Split layers' codes
                    latent_code_layers_tuple = torch.split(tensor=latent_code,
                                                           split_size_or_sections=latent_code.shape[1] * [1],
                                                           dim=1)
                    # Find shift per layer
                    latent_shift = []
                    for layer_idx in range(STYLEGAN_LAYERS[self.params.gan]):
                        if layer_idx < self.params.stylegan_layer:
                            layer_shift = target_shift_magnitudes.reshape(-1, 1) * \
                                          latent_support_sets[layer_idx](support_sets_mask,
                                                                         latent_code_layers_tuple[layer_idx].squeeze(1))
                            latent_shift.append(layer_shift.unsqueeze(1))
                        else:
                            latent_shift.append(torch.zeros(latent_code.shape[0], 1, latent_code.shape[2]))
                    # REVIEW: Upload to CUDA?
                    latent_shift = torch.cat(latent_shift, dim=1).to('cuda' if self.use_cuda else 'cpu')
                ...
                latent_shift = target_shift_magnitudes.reshape(-1, 1) * latent_support_sets[0](support_sets_mask,
                                                                                               latent_code)
            ...
            ############################################################################################################
            img_shifted = None
            if ('stylegan' in self.params.gan) and (self.params.stylegan_space == 'S'):
                raise NotImplementedError
            ...
            ############################################################################################################
            elif "proposed-no-warping" in self.params.vl_sim:
                # Calculate the orthogonal projection of the difference of the VL image features, i.e., the manipulated
                # (vl_img_shifted) minus the original (vl_img) onto the tangent space T_{vl_img}S^{n-1}
            ...
            if iteration % self.params.ckp_freq == 0:
                latent_support_sets_dict = dict()
                for layer_idx in range(len(latent_support_sets)):
                    latent_support_sets_dict.update(
                        {'layer_{}'.format(layer_idx): latent_support_sets[layer_idx].state_dict()})
                # Build checkpoint dict
                checkpoint_dict = {
                    'iter': iteration,
                    'latent_support_sets': latent_support_sets_dict,
        ...
        for layer_idx in range(len(latent_support_sets)):
            latent_support_sets_dict.update(
                {'layer_{}'.format(layer_idx): latent_support_sets[layer_idx].state_dict()})
        torch.save(latent_support_sets_init_dict, latent_support_sets_model_filename)
